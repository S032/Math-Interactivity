...
Engine3D::Engine3D(int newWinWidth, int newWinHeight, const char *newWinTitle,
                   bool polygonal)
    : winWidth(newWinWidth), winHeight(newWinHeight), winTitle(newWinTitle) {
  cameraProps_t camProps;
  camProps.FOV = 90.0f;
  camProps.zFar = 100.0f;
  camProps.speed = 0.05f;
  camProps.sensivity = 1.0f;
  camProps.pos = glm::vec3(0.0f, 0.0f, 0.0f);
...
void Engine3D::primitiveDrawer(glm::mat4 proj_view, glm::vec3 camPos,
                               glm::vec3 lightPos) {
  std::vector<dot_t> dots = dotHandler->getDotsArr();

  // Object3D sphere(objectType::SPHERE, glm::vec4(0.2f, 0.2f, 1.0f, 1.0f),
  // shaderProgram);
...
void Engine3D::start() {
  ...
  Object3D lightcube(objectType::CUBE, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f),
                     shaderProgram);
  Object3D field(objectType::CUBE, glm::vec4(1.0f, 1.0f, 1.0f, 0.2f),
                 shaderProgram);
  ...
  while (!glfwWindowShouldClose(win)) {
    ...
    lightcube.setPos(glm::vec3(glm::cos(glfwGetTime() * 2) * 4,
                               glm::sin(glfwGetTime() * 2) * 4,
                               glm::sin(glfwGetTime() * 2) * 4));
